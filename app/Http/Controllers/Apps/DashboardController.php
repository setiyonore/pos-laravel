...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        //count sales today
        $count_sales_today = Transaction::query()
            ->whereDay('created_at',$day)->count();

        //sum sales today
        $sum_sales_today = Transaction::query()
            ->whereDay('created_at',$day)->sum('grand_total');
        //sum profits today
        $sum_profits_today = Profit::query()
            ->whereDay('created_at',$day)->sum('total');
        //get produk limit stock
